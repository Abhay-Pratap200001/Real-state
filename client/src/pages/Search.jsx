  ...
  useEffect(()=>{
    ...
    //step4
     //if new value exixt keep update it other wise keep it default before they are
      if ( searchTermFromUrl || typeFromUrl || parkingFromUrl || furnishedFromUrl || offerFromUrl || sortFromUrl ||orderFromUrl){// if any these value is available in url update sidebarstate
        setSidebardata({ 
...
//step1
  const handleChange = (e) =>{  
    ...
    if (e.target.id === 'searchTerm') { //if id is equal to search
    ...
    if (e.target.id === 'parking' || e.target.id === 'furnished' || e.target.id === 'offer'){
      setSidebardata({...sidebardata, [e.target.id]: e.target.checked || e.target.checked === 'true' ? true : false,}); // if its bollena true or string true do it true or do false
    }

    if (e.target.id === 'sort_order') { // if id is true do according to condition run this
    ...
    <div className="flex flex-col md:flex-row">
      <div className="p-7  border-b-2 md:border-r-2 md:min-h-screen"> {/* {left side div} */}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8 mt-20 ">
          ...
          <div className="flex gap-2 flex-wrap items-center"> {/* {Check boxes} */}
            ...
            <div className="flex gap-2">
              <input type="checkbox" id="sale" className="w-5" 
                onChange={handleChange}
                checked={sidebardata.type === 'sale'}/> {/*is equal*/} 
              <span>Sale</span>
            </div>
            ...
            <div className="flex gap-2">
              <input type="checkbox" id="offer" className="w-5" 
                onChange={handleChange}
                checked={sidebardata.offer}/>  {/* is true*/}
              <span>Offer</span>
            </div>
          </div>
           ...
           <div className='flex items-center gap-3'>{/* {sorting section} */}
